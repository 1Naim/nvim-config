local function lsp_config(event)
    ...
    -- Show signature
    map('<C-k>', vim.lsp.buf.signature_help, 'Show signature', 'i')
    ...
    if client then
        ...
        if client_supports_method(client, vim.lsp.protocol.textDocument_completion, event.buf) then
            ...
            map('<C-space>', vim.lsp.completion.get, 'Trigger Autocomplete', 'i')

            -- Tab to select next item or insert tab
            ...
            -- Enter to confirm selection (or just insert newline)
